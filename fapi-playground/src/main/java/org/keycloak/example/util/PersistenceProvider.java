...
public class PersistenceProvider {
    ...
    /**
     * Persists the attestation key (and optional certificate chain) from {@code oid4vciCtx}
     * to {@code data/attestation-data.json}. Called each time a new attestation key is generated.
     */
    public static void saveAttestationKey(OID4VCIContext oid4vciCtx) {
        ...
        try {
            ...
            // Persist certificate chain (if present – set by 'Generate attestation certificates')
            List<X509Certificate> certChain = oid4vciCtx.getAttestationCertChain();
            if (certChain != null && !certChain.isEmpty()) {
                ArrayNode certsNode = root.putArray("certificateChain");
                for (X509Certificate cert : certChain) {
                    certsNode.add(Base64.getEncoder().encodeToString(cert.getEncoded()));
                }
            }
    ...
    /**
     * Loads the attestation key (and optional certificate chain) from
     * {@code data/attestation-data.json} (if present) into {@code oid4vciCtx}.
    ...
    public static void loadAttestationKey(OID4VCIContext oid4vciCtx) {
        ...
        try {
            ...
            // Restore certificate chain if persisted
            if (root.has("certificateChain")) {
                java.security.cert.CertificateFactory cf = java.security.cert.CertificateFactory.getInstance("X.509");
                List<X509Certificate> certChain = new ArrayList<>();
                for (com.fasterxml.jackson.databind.JsonNode certNode : root.get("certificateChain")) {
                    byte[] certBytes = Base64.getMimeDecoder().decode(certNode.asText());
                    X509Certificate cert = (X509Certificate) cf.generateCertificate(new java.io.ByteArrayInputStream(certBytes));
                    certChain.add(cert);
                }
                kw.setCertificate(certChain.get(0));
                kw.setCertificateChain(certChain);

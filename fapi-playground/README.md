...
Once the credential is obtained, then (in case of `jwt` proof was used), you can also notice that sd-jwt credential will be bound to the
...
1. Follow the same steps as for the `attestation` proof type (generate the attestation key, configure the Trusted Key IDP in Keycloak, and link it to the client). See the section *Additional steps for the `attestation` proof type* above.
...
The `attestation` proof type requires that Keycloak has a **Trusted Key** identity provider configured with the attestation public key. If you skip this setup, Keycloak will reject the credential request with:

```
"error" : "invalid_proof"
"error_description" : "Key with kid '...' not found in trusted key registry"
```
...
2. In the Keycloak admin console for realm `test`, go to **Identity Providers** and create a new **Trusted Key** identity provider.
   Paste the JWKS copied from the previous step into the JWKS field of the provider configuration.
